...
module private Helpers =
    ...
    let argSplitter expected defVal (args:string) =
        ...
        else Array.concat [|vals; Array.create (expected - vals.Length) defVal|]
    ...
    let escapeCode arglist = function
        ...
        | "K" -> EraseInLine (arglist |> oneArg 0)
        | _ as letter -> failwithf "Unsupported option %s" letter
    ...
    let rec parse (s:string) start (ms:MatchCollection) idx = seq {
        if idx >= ms.Count then
            if start < s.Length then yield Text (s.Substring(start))
             ...
             yield escapeCode m.Groups.[1].Value m.Groups.[2].Value 
